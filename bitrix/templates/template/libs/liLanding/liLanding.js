		...
		init: function (options) {
			...
			return this.each(function () {
				...
				$('a',el).on('click',function(){
					var linkItem = $(this);
					if(!linkItem.is('.nav__item_select')){
						...
						if(linkTarget.length){
							$('html, body').stop(true).animate({scrollTop:(linkTargetPos-parseFloat(p.topMargin) - 80)},animDuration*p.speedFactor,function(){
					...
					scrollId = setTimeout(function(){
						$('.anchor').each(function(){
							...
							if(windowPos > landingItemTop && windowPos < (landingItemTop + landingItemHeight)){
								if(!linkEl.is('.nav__item_select')){
									linkEl.addClass('nav__item_select');
									if (p.show !== undefined) {
								...
								if(linkEl.is('.nav__item_select')){
									linkEl.removeClass('nav__item_select');
									if (p.hide !== undefined) {
